...
.DESCRIPTION
    ...
    - Mouse & Keyboard Latency Engine (Buffer Queue 32 Anti-Bufferbloat, eSports FilterKeys Zero Delay 200/20ms, USB xHCI MSI Mode & High Priority, Global Anti-Selective Suspend, USB D3 Sleep Off, MarkC 1:1 Linear Curve, Instant Desktop Delay, Win11 DWM Input Suppression Fix, Anti-Telemetry)
...
param(
    ...
    [switch]$AMD,
    [switch]$Boost,
    [Alias("Input", "Keys", "Mouse", "Latency")][switch]$InputTuning,
    [switch]$Browser,
    [switch]$Auto,
    [switch]$DryRun,
    [switch]$Analyze,
...
# Native Win32 Input & Latency Helper (FilterKeys & Keyboard Response)
if (-not ([System.Management.Automation.PSTypeName]'RBInputHelper'.Type)) {
    Add-Type -TypeDefinition @"
using System;
using System.Runtime.InteropServices;

public class RBInputHelper {
    [StructLayout(LayoutKind.Sequential)]
    ...
    public static extern bool SystemParametersInfoIntPtr(uint uiAction, uint uiParam, IntPtr pvParam, uint fWinIni);

    public static bool ApplyFilterKeys(uint flags, uint waitMSec, uint delayMSec, uint repeatMSec, uint bounceMSec) {
        try {
            FILTERKEYS fk = new FILTERKEYS();
            fk.cbSize = (uint)Marshal.SizeOf(typeof(FILTERKEYS));
            fk.dwFlags = flags;
            fk.iWaitMSec = waitMSec;
            fk.iDelayMSec = delayMSec;
            fk.iRepeatMSec = repeatMSec;
            fk.iBounceMSec = bounceMSec;
            return SystemParametersInfo(SPI_SETFILTERKEYS, fk.cbSize, ref fk, SPIF_UPDATEINIFILE | SPIF_SENDCHANGE);
        } catch {
            return false;
        }
    }

    public static bool ApplyKeyboardSpeed(uint delay, uint speed) {
        try {
            bool r1 = SystemParametersInfoIntPtr(SPI_SETKEYBOARDDELAY, delay, IntPtr.Zero, SPIF_UPDATEINIFILE | SPIF_SENDCHANGE);
            bool r2 = SystemParametersInfoIntPtr(SPI_SETKEYBOARDSPEED, speed, IntPtr.Zero, SPIF_UPDATEINIFILE | SPIF_SENDCHANGE);
            return r1 && r2;
        } catch {
            return false;
        }
...
foreach ($rawArg in $allPassed) {
    ...
    switch -Regex ($token) {
        ...
        '^(input|latency|mouse|keyboard|keys)$' { $InputTuning = $true }
...
# 3B. USB xHCI Controller MSI Mode & High Priority Engine
function Enable-UsbXhciMsiAndPriority {
    param([string]$Priority = "High")
    if ($Script:DryRun) {
        Write-Log "[DRY] Would configure MSI Mode and High Device Priority for PCI USB xHCI Controllers (Priority=$Priority)" "DRY"
        return
    }
    try {
        $prioVal = if ($Priority -eq "High") { 3 } elseif ($Priority -eq "Normal") { 2 } elseif ($Priority -eq "Low") { 1 } else { 0 }
        ...
        foreach ($ctrl in $usbControllers) {
            ...
            if ($status -and $status -ne "OK") {
                Write-Log "SKIP USB Controller MSI for '$name': Device status is '$status'" "WARN"
                continue
            }

            if ($pnpId -and $pnpId -match "^PCI\\") {
                $pciKeyPath = "HKLM:\SYSTEM\CurrentControlSet\Enum\$pnpId"
                if (Test-Path $pciKeyPath) {
                    $msiKey = Join-Path $pciKeyPath "Device Parameters\Interrupt Management\MessageSignaledInterruptProperties"
                    $affKey = Join-Path $pciKeyPath "Device Parameters\Interrupt Management\Affinity Policy"
                    ...
                    Write-Log "OK  PCI USB Controller MSI Mode Enabled for '$name' (MSISupported=1, Affinity Priority=$prioVal)" "SUCCESS"
                    $found++
                }
            }
        }
        if ($found -eq 0) {
            Write-Log "INFO No PCI USB xHCI controllers modified." "INFO"
        }
        Write-Status "USB xHCI MSI Mode Enabled ($found controller(s))" $Script:C.Green
    } catch {
        Write-Log "FAIL Enable-UsbXhciMsiAndPriority -- $_" "ERROR"
    }
}

# 3C. Emergency Mouse, Keyboard & USB Input Latency Reset Engine
function Reset-InputToDefault {
    if ($Script:DryRun) {
        Write-Log "[DRY] Would perform Reset-InputToDefault (restoring stock mouse, keyboard & USB settings)" "DRY"
        return $true
    }
    Write-Log "================================================================================" "INFO"
    Write-Log "            MOUSE, KEYBOARD & USB LATENCY RESET TO WINDOWS DEFAULT              " "INFO"
    Write-Log "================================================================================" "INFO"
    Write-Status "Resetting Mouse, Keyboard & USB Settings..." $Script:C.Yellow

    # 1. Reset MouseDataQueueSize to 100
    try {
        ...
        Write-Log "OK  Restored MouseDataQueueSize to default (100)" "SUCCESS"
    } catch { Write-Log "WARN Reset MouseDataQueueSize: $_" "WARN" }

    # 2. Reset KeyboardDataQueueSize to 100
    try {
        Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\kbdclass\Parameters" -Name "KeyboardDataQueueSize" -Value 100 -Type DWord -ErrorAction SilentlyContinue
        Write-Log "OK  Restored KeyboardDataQueueSize to default (100)" "SUCCESS"
    } catch { Write-Log "WARN Reset KeyboardDataQueueSize: $_" "WARN" }

    # 3. Reset FilterKeys to Windows Default (Off, 1000/1000/500ms)
    try {
        $respKey = "HKCU:\Control Panel\Accessibility\Keyboard Response"
        if (Test-Path $respKey) {
            Set-ItemProperty -Path $respKey -Name "Flags" -Value "126" -Type String -ErrorAction SilentlyContinue
            Set-ItemProperty -Path $respKey -Name "DelayBeforeAcceptance" -Value "1000" -Type String -ErrorAction SilentlyContinue
            Set-ItemProperty -Path $respKey -Name "AutoRepeatDelay" -Value "1000" -Type String -ErrorAction SilentlyContinue
            Set-ItemProperty -Path $respKey -Name "AutoRepeatRate" -Value "500" -Type String -ErrorAction SilentlyContinue
            Set-ItemProperty -Path $respKey -Name "BounceTime" -Value "0" -Type String -ErrorAction SilentlyContinue
        }
        if ([System.Management.Automation.PSTypeName]'RBInputHelper'.Type) {
            [RBInputHelper]::ApplyFilterKeys(126, 1000, 1000, 500, 0) | Out-Null
        }
        Write-Log "OK  Restored FilterKeys to Windows default (Off, 1000ms delay, 500ms rate)" "SUCCESS"
    } catch { Write-Log "WARN Reset FilterKeys: $_" "WARN" }

    # 4. Reset Native Win32 Keyboard Delay & Speed
    try {
        $kbdKey = "HKCU:\Control Panel\Keyboard"
        if (Test-Path $kbdKey) {
            Set-ItemProperty -Path $kbdKey -Name "KeyboardDelay" -Value "1" -Type String -ErrorAction SilentlyContinue
            Set-ItemProperty -Path $kbdKey -Name "KeyboardSpeed" -Value "31" -Type String -ErrorAction SilentlyContinue
        }
        if ([System.Management.Automation.PSTypeName]'RBInputHelper'.Type) {
            [RBInputHelper]::ApplyKeyboardSpeed(1, 31) | Out-Null
        }
        Write-Log "OK  Restored Win32 Keyboard repeat delay to default (Delay=1, Speed=31)" "SUCCESS"
    } catch { Write-Log "WARN Reset Keyboard repeat: $_" "WARN" }

    # 5. Reset CSRSS RIT Priority
    try {
        $csrssKey = "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\csrss.exe\PerfOptions"
        if (Test-Path $csrssKey) {
            Remove-ItemProperty -Path $csrssKey -Name "CpuPriorityClass" -ErrorAction SilentlyContinue
            Remove-ItemProperty -Path $csrssKey -Name "IoPriority" -ErrorAction SilentlyContinue
            Write-Log "OK  Removed CSRSS PerfOptions CPU & IO priority overrides" "SUCCESS"
        }
    } catch { Write-Log "WARN Reset CSRSS: $_" "WARN" }

    # 6. Reset USB xHCI Controller MSI Priority
    try {
        $usbControllers = Get-CimInstance Win32_USBController -ErrorAction SilentlyContinue
        foreach ($ctrl in $usbControllers) {
            $pnpId = $ctrl.DeviceID
            if ($pnpId -and $pnpId -match "^PCI\\") {
                $affKey = "HKLM:\SYSTEM\CurrentControlSet\Enum\$pnpId\Device Parameters\Interrupt Management\Affinity Policy"
                if (Test-Path $affKey) {
                    Remove-ItemProperty -Path $affKey -Name "DevicePriority" -ErrorAction SilentlyContinue
                }
            }
        }
        Write-Log "OK  Restored USB Controller interrupt affinity priorities" "SUCCESS"
    } catch { Write-Log "WARN Reset USB MSI: $_" "WARN" }

    # 7. Restore USB Selective Suspend
    try {
        ...
        Remove-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\USBHUB3\Parameters" -Name "DisableSelectiveSuspend" -ErrorAction SilentlyContinue
        Write-Log "OK  Restored USB selective suspend settings" "SUCCESS"
    } catch { Write-Log "WARN Reset USB Selective Suspend: $_" "WARN" }

    # 8. Restore USB Device Power Management
    try {
        $devParams = Get-ChildItem -Path "HKLM:\SYSTEM\CurrentControlSet\Enum\USB" -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.PSChildName -eq "Device Parameters" }
        foreach ($dp in $devParams) {
            Set-ItemProperty -Path $dp.PSPath -Name "EnhancedPowerManagementEnabled" -Value 1 -Type DWord -ErrorAction SilentlyContinue
            Set-ItemProperty -Path $dp.PSPath -Name "AllowIdleIrpInD3" -Value 1 -Type DWord -ErrorAction SilentlyContinue
        }
        Write-Log "OK  Restored USB Device Power Management D3 to default" "SUCCESS"
    } catch { Write-Log "WARN Reset USB D3: $_" "WARN" }

    # 9. Restore Default Windows Mouse Acceleration Curves
    try {
        $mKey = "HKCU:\Control Panel\Mouse"
        if (Test-Path $mKey) {
            Set-ItemProperty -Path $mKey -Name "MouseSpeed" -Value "1" -Type String -ErrorAction SilentlyContinue
            Set-ItemProperty -Path $mKey -Name "MouseThreshold1" -Value "6" -Type String -ErrorAction SilentlyContinue
            Set-ItemProperty -Path $mKey -Name "MouseThreshold2" -Value "10" -Type String -ErrorAction SilentlyContinue

            [byte[]]$defaultX = @(0,0,0,0,0,0,0,0, 21,110,0,0,0,0,0,0, 0,64,1,0,0,0,0,0, 0,128,2,0,0,0,0,0, 0,0,4,0,0,0,0,0)
            [byte[]]$defaultY = @(0,0,0,0,0,0,0,0, 184,11,1,0,0,0,0,0, 112,23,2,0,0,0,0,0, 40,35,3,0,0,0,0,0, 224,46,4,0,0,0,0,0)

            Set-ItemProperty -Path $mKey -Name "SmoothMouseXCurve" -Value $defaultX -Type Binary -ErrorAction SilentlyContinue
            Set-ItemProperty -Path $mKey -Name "SmoothMouseYCurve" -Value $defaultY -Type Binary -ErrorAction SilentlyContinue
            Write-Log "OK  Restored default Windows SmoothMouse curves and acceleration" "SUCCESS"
        }
    } catch { Write-Log "WARN Reset Mouse Curves: $_" "WARN" }

    # 10. Restore Desktop Mouse & Menu Delay
    try {
        $dtKey = "HKCU:\Control Panel\Desktop"
        if (Test-Path $dtKey) {
            Set-ItemProperty -Path $dtKey -Name "MouseHoverTime" -Value "400" -Type String -ErrorAction SilentlyContinue
            Set-ItemProperty -Path $dtKey -Name "MenuShowDelay" -Value "400" -Type String -ErrorAction SilentlyContinue
            Write-Log "OK  Restored MouseHoverTime (400ms) and MenuShowDelay (400ms)" "SUCCESS"
        }
    } catch { Write-Log "WARN Reset Desktop delays: $_" "WARN" }

    # 11. Remove DWM DisableInputSuppression
    try {
        $dwmKey = "HKLM:\SOFTWARE\Microsoft\Windows\DWM"
        if (Test-Path $dwmKey) {
            Remove-ItemProperty -Path $dwmKey -Name "DisableInputSuppression" -ErrorAction SilentlyContinue
            Write-Log "OK  Removed DWM DisableInputSuppression override" "SUCCESS"
        }
    } catch { Write-Log "WARN Reset DWM InputSuppression: $_" "WARN" }

    # 12. Restore Input Personalization
    try {
        $ipUser = "HKCU:\Software\Microsoft\InputPersonalization"
        if (Test-Path $ipUser) {
            Remove-ItemProperty -Path $ipUser -Name "RestrictImplicitInkCollection" -ErrorAction SilentlyContinue
            Remove-ItemProperty -Path $ipUser -Name "RestrictImplicitTextCollection" -ErrorAction SilentlyContinue
        }
        $ipPol = "HKLM:\SOFTWARE\Policies\Microsoft\InputPersonalization"
        if (Test-Path $ipPol) {
            Remove-ItemProperty -Path $ipPol -Name "RestrictImplicitInkCollection" -ErrorAction SilentlyContinue
            Remove-ItemProperty -Path $ipPol -Name "RestrictImplicitTextCollection" -ErrorAction SilentlyContinue
        }
        Write-Log "OK  Restored Input Personalization defaults" "SUCCESS"
    } catch { Write-Log "WARN Reset Input Personalization: $_" "WARN" }

    Write-Log "OK  Mouse, Keyboard & USB Latency Settings successfully restored to Windows default!" "SUCCESS"
    Write-Status "Input & USB Settings Restored to Default" $Script:C.Green
    return $true
}
...
Add-Tweak @{ Id="BST13"; Name="CSRSS Client-Server Runtime High Priority"; Cat="Gaming Boost"; Risk="Low"
    Desc="Elevates CSRSS (Client Server Runtime) CPU priority class to 3 (Above Normal) and I/O priority to High to eliminate mouse cursor lag without preempting game threads."
    Action={
        Set-RegValue "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\csrss.exe\PerfOptions" "CpuPriorityClass" 3 -Desc "CSRSS Above Normal CPU Priority"
...
# -----------------------------------------------------------------------------
# 3B. MOUSE & KEYBOARD LATENCY OPTIMIZATIONS (/input)
# -----------------------------------------------------------------------------
Add-Tweak @{ Id="INP01"; Name="Mouse Buffer Queue Reduction (MouseDataQueueSize 32)"; Cat="Mouse & Keyboard"; Risk="Low"
    Desc="Reduces mouclass packet buffer queue from default 100 to 32 to eliminate input bufferbloat and cursor delivery lag."
    Action={
        Set-RegValue "HKLM:\SYSTEM\CurrentControlSet\Services\mouclass\Parameters" "MouseDataQueueSize" 32 -Desc "Mouse Data Queue Size 32"
    }
}
Add-Tweak @{ Id="INP02"; Name="Keyboard Buffer Queue Reduction (KeyboardDataQueueSize 32)"; Cat="Mouse & Keyboard"; Risk="Low"
    Desc="Reduces kbdclass packet buffer queue from default 100 to 32 to eliminate keyboard stroke queuing delays."
    Action={
        Set-RegValue "HKLM:\SYSTEM\CurrentControlSet\Services\kbdclass\Parameters" "KeyboardDataQueueSize" 32 -Desc "Keyboard Data Queue Size 32"
    }
}
Add-Tweak @{ Id="INP03"; Name="eSports FilterKeys Latency Response (Zero Delay, 200/20ms)"; Cat="Mouse & Keyboard"; Risk="Low"
    Desc="Configures competitive eSports FilterKeys (0ms delay, 200ms repeat delay, 20ms repeat rate) with instant Win32 live activation."
    Action={
        $fkKey = "HKCU:\Control Panel\Accessibility\Keyboard Response"
        Set-RegValue $fkKey "Flags" "27" -Type String -Desc "FilterKeys Flags 27"
        Set-RegValue $fkKey "DelayBeforeAcceptance" "0" -Type String -Desc "DelayBeforeAcceptance 0"
        Set-RegValue $fkKey "AutoRepeatDelay" "200" -Type String -Desc "AutoRepeatDelay 200"
        Set-RegValue $fkKey "AutoRepeatRate" "20" -Type String -Desc "AutoRepeatRate 20"
        Set-RegValue $fkKey "BounceTime" "0" -Type String -Desc "BounceTime 0"
        if (-not $Script:DryRun -and ([System.Management.Automation.PSTypeName]'RBInputHelper'.Type)) {
            [RBInputHelper]::ApplyFilterKeys(27, 0, 200, 20, 0) | Out-Null
            Write-Log "OK  Live eSports FilterKeys applied via SystemParametersInfo" "SUCCESS"
        }
    }
}
Add-Tweak @{ Id="INP04"; Name="Win32 Native Keyboard Repeat Delay & Speed"; Cat="Mouse & Keyboard"; Risk="Low"
    Desc="Sets native Windows keyboard repeat delay to minimum (0) and repeat speed to maximum (31) for immediate keystroke response."
    Action={
        Set-RegValue "HKCU:\Control Panel\Keyboard" "KeyboardDelay" "0" -Type String -Desc "KeyboardDelay 0"
        Set-RegValue "HKCU:\Control Panel\Keyboard" "KeyboardSpeed" "31" -Type String -Desc "KeyboardSpeed 31"
        if (-not $Script:DryRun -and ([System.Management.Automation.PSTypeName]'RBInputHelper'.Type)) {
            [RBInputHelper]::ApplyKeyboardSpeed(0, 31) | Out-Null
            Write-Log "OK  Live Win32 Keyboard Delay/Speed applied via SystemParametersInfo" "SUCCESS"
        }
    }
}
Add-Tweak @{ Id="INP05"; Name="CSRSS RIT Engine Priority (High CPU & High I/O)"; Cat="Mouse & Keyboard"; Risk="Low"
    Desc="Elevates CSRSS Raw Input Thread (RIT) CPU priority to High (3) and I/O priority to High (3) to prevent input drops under heavy CPU load."
    Action={
        Set-RegValue "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\csrss.exe\PerfOptions" "CpuPriorityClass" 3 -Desc "CSRSS High CPU Priority"
        Set-RegValue "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\csrss.exe\PerfOptions" "IoPriority" 3 -Desc "CSRSS High I/O Priority"
    }
}
Add-Tweak @{ Id="INP06"; Name="USB xHCI Controller MSI Mode & High Device Priority"; Cat="Mouse & Keyboard"; Risk="Low"
    Desc="Switches PCI USB xHCI host controllers to Message Signaled Interrupts (MSI Mode) with High device interrupt priority."
    Action={
        Enable-UsbXhciMsiAndPriority -Priority "High"
    }
}
Add-Tweak @{ Id="INP07"; Name="Global USB Anti-Selective Suspend"; Cat="Mouse & Keyboard"; Risk="Low"
    Desc="Disables USB selective suspend across core USB and USBHUB3 drivers to maintain steady 1000Hz - 8000Hz polling rates."
    Action={
        Set-RegValue "HKLM:\SYSTEM\CurrentControlSet\Services\USB" "DisableSelectiveSuspend" 1 -Desc "Disable USB Selective Suspend"
        Set-RegValue "HKLM:\SYSTEM\CurrentControlSet\Services\USBHUB3\Parameters" "DisableSelectiveSuspend" 1 -Desc "Disable USBHUB3 Selective Suspend"
        if (-not $Script:DryRun) {
...
Add-Tweak @{ Id="INP08"; Name="USB Device Power Saving D3 Off"; Cat="Mouse & Keyboard"; Risk="Low"
    Desc="Disables idle D3 sleep states and enhanced power management across all connected and enumerated USB peripheral devices."
    Action={
        if ($Script:DryRun) {
            Write-Log "[DRY] Would disable USB Idle D3 and EnhancedPowerManagement across USB devices" "DRY"
            return
        }
        $params = Get-ChildItem -Path "HKLM:\SYSTEM\CurrentControlSet\Enum\USB" -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.PSChildName -eq "Device Parameters" }
        $count = 0
        foreach ($dp in $params) {
            Set-ItemProperty -Path $dp.PSPath -Name "EnhancedPowerManagementEnabled" -Value 0 -Type DWord -ErrorAction SilentlyContinue
            Set-ItemProperty -Path $dp.PSPath -Name "AllowIdleIrpInD3" -Value 0 -Type DWord -ErrorAction SilentlyContinue
            Set-ItemProperty -Path $dp.PSPath -Name "DeviceSelectiveSuspended" -Value 0 -Type DWord -ErrorAction SilentlyContinue
            $count++
        }
        Write-Log "OK  USB Device D3 Power Saving disabled across $count USB device entries" "SUCCESS"
    }
}
Add-Tweak @{ Id="INP09"; Name="1:1 Raw Mouse Input & Linear Curve (MarkC Hex)"; Cat="Mouse & Keyboard"; Risk="Low"
    Desc="Disables mouse acceleration curve and applies MarkC 1:1 linear curve (MouseSpeed=0, Thresholds=0, Linear Hex Curves)."
    Action={
        $mKey = "HKCU:\Control Panel\Mouse"
        Set-RegValue $mKey "MouseSpeed" "0" -Type String -Desc "Mouse Speed 0"
        Set-RegValue $mKey "MouseThreshold1" "0" -Type String -Desc "Mouse Threshold1 0"
        Set-RegValue $mKey "MouseThreshold2" "0" -Type String -Desc "Mouse Threshold2 0"
        [byte[]]$linearCurve = @(
        ...
        Set-RegValue $mKey "SmoothMouseXCurve" $linearCurve -Type Binary -Desc "MarkC 1:1 Linear SmoothMouseXCurve"
        Set-RegValue $mKey "SmoothMouseYCurve" $linearCurve -Type Binary -Desc "MarkC 1:1 Linear SmoothMouseYCurve"
    }
}
Add-Tweak @{ Id="INP10"; Name="Instant Desktop Mouse & Menu Delay"; Cat="Mouse & Keyboard"; Risk="Low"
    Desc="Sets MouseHoverTime to 10ms and MenuShowDelay to 0ms for instantaneous context menus and hover responsiveness."
    Action={
        Set-RegValue "HKCU:\Control Panel\Desktop" "MouseHoverTime" "10" -Type String -Desc "MouseHoverTime 10ms"
        Set-RegValue "HKCU:\Control Panel\Desktop" "MenuShowDelay" "0" -Type String -Desc "MenuShowDelay 0ms"
    }
}
Add-Tweak @{ Id="INP11"; Name="Windows 11 DWM High Polling Input Suppression Fix"; Cat="Mouse & Keyboard"; Risk="Low"
    Desc="Prevents Windows 11 Desktop Window Manager from suppressing or throttling high polling rate (1K/4K/8KHz) mouse inputs."
    Action={
        Set-RegValue "HKLM:\SOFTWARE\Microsoft\Windows\DWM" "DisableInputSuppression" 1 -Desc "Disable DWM Input Suppression"
    }
}
Add-Tweak @{ Id="INP12"; Name="Disable Input Telemetry & Handwriting Harvesting"; Cat="Mouse & Keyboard"; Risk="Low"
    Desc="Disables Windows background inking and typing data collection, contact harvesting, and dictionary uploading."
    Action={
        Set-RegValue "HKCU:\Software\Microsoft\InputPersonalization" "RestrictImplicitInkCollection" 1 -Desc "Disable User Ink Collection"
        Set-RegValue "HKCU:\Software\Microsoft\InputPersonalization" "RestrictImplicitTextCollection" 1 -Desc "Disable User Text Collection"
        Set-RegValue "HKCU:\Software\Microsoft\InputPersonalization\TrainedDataStore" "HarvestContacts" 0 -Desc "Disable Contact Harvesting"
        Set-RegValue "HKLM:\SOFTWARE\Policies\Microsoft\InputPersonalization" "RestrictImplicitInkCollection" 1 -Desc "Policy Disable Ink Collection"
        Set-RegValue "HKLM:\SOFTWARE\Policies\Microsoft\InputPersonalization" "RestrictImplicitTextCollection" 1 -Desc "Policy Disable Text Collection"
        Set-RegValue "HKCU:\Software\Microsoft\Personalization\Settings" "AcceptedPrivacyPolicy" 0 -Desc "Revoke Input Privacy Policy"
    }
}
...
# (Network & Latency tweaks removed per user request)
...
$Script:Presets = @{
    "All-in-One Intel Pro" = @{
        Ids = @("INTEL01","INTEL02","INTEL03","INTEL04","INTEL05","INTEL06","INTEL07","INTEL08","INTEL09","INTEL10","BST01","BST02","BST03","BST04","BST05","BST06","BST07","BST08","BST09","BST10","BST11","BST12","BST13","BST14","BRW01","BRW02","BRW03","BRW04","BRW05","PERF01","PERF02","PERF03","PERF04","PERF09","P01","P02","AI01","AI02","AI04")
        Tier = 1
    }
    "All-in-One AMD Pro" = @{
        Ids = @("AMD01","AMD02","AMD03","AMD04","AMD05","AMD06","AMD07","AMD08","BST01","BST02","BST03","BST04","BST05","BST06","BST07","BST08","BST09","BST10","BST11","BST12","BST13","BST14","BRW01","BRW02","BRW03","BRW04","BRW05","PERF01","PERF02","PERF03","PERF04","PERF09","P01","P02","AI01","AI02","AI04")
        Tier = 1
    }
    "Auto-Detect Pro" = @{
    ...
    "Gaming / FPS Boost" = @{
        Ids = @("BST01","BST02","BST03","BST04","BST05","BST06","BST07","BST08","BST09","BST10","BST11","BST12","BST13","BST14","BRW01","BRW02","BRW05","PERF01","PERF02","PERF03","PERF04","PERF09","VIS01","VIS02","AI01","AI02","AI04")
        Tier = 1
    }
    "Mouse & Keyboard Latency" = @{
        Ids = @("INP01","INP02","INP03","INP04","INP05","INP06","INP07","INP08","INP09","INP10","INP11","INP12")
        Tier = 1
    }
    "Browser & Light Boost" = @{
        Ids = @("BRW01","BRW02","BRW03","BRW04","BRW05","PERF03","PERF04","P01","P02","AI01","AI04")
        Tier = 1
    }
    "Privacy-First" = @{
        Ids = @("P01","P02","P03","P04","P05","P06","AI01","AI02","AI03","AI04","BRW03","SVC01","SVC02","SVC03")
        Tier = 2
    }
}
...
$Script:PresetDesc = @{
    ...
    "Mouse & Keyboard Latency" = "Reduces mouse & keyboard latency to hardware minimum: 32 queue buffers, eSports FilterKeys, USB xHCI MSI mode, 1:1 MarkC curve, and DWM suppression fix."
...
# =============================================================================
if ($Intel -or $AMD -or $Boost -or $InputTuning -or $Browser -or $Auto -or $Analyze -or $Debloat -or $ResetGPU -or $Help -or ($DryRun -and $allPassed.Count -gt 0)) {
    ...
    if ($Help) {
        ...
        Write-Host "    -Input,   /input    : Mouse & Keyboard Latency (Anti-Bufferbloat 32, FilterKeys, USB xHCI MSI, 1:1 MarkC)" -ForegroundColor White
    ...
    if ($InputTuning) {
        Write-Host "[*] Selected Mouse & Keyboard Latency Profile (/input)..." -ForegroundColor Cyan
        foreach ($id in $Script:Presets["Mouse & Keyboard Latency"].Ids) {
            if (-not $tweakIds.Contains($id)) { $tweakIds.Add($id) }
        }
    }
    if ($Browser) {
...
function Build-MainGUI {
    ...
    $tc.ItemSize = New-Object System.Drawing.Size(98, 34)
    ...
    $presetList = @(
        ...
        @{ Name="Mouse & Keyboard Latency"; Col=$Script:C.Sapphire }
    ...
    foreach ($p in $presetList) {
        ...
        $pb = New-Btn "  $pname" 14 $py 204 38 -BG $pcolor -FG $Script:C.Crust -Fnt (New-Object System.Drawing.Font("Segoe UI", 9.5, [System.Drawing.FontStyle]::Bold))
        $pb.Tag = $pname
        $pb.Add_Click({
            param($s2, $e2)
            $pn = [string]$s2.Tag
    ...
    # =========================================================================
    # TAB 4B: MOUSE & KEYBOARD LATENCY (/input)
    # =========================================================================
    $inputNote = "Input Latency Engine: Tối ưu hàng đợi đệm (Anti-Bufferbloat 32), eSports FilterKeys (200/20ms), USB xHCI MSI Mode, MarkC 1:1 Raw Input, khắc phục DWM Input Suppression."
    $inputTab = Build-TweakTab "  Mouse & Keys  " "Mouse & Keyboard" $Script:C.Sapphire "Apply Input Tweaks" $inputNote

    # Add Quick Actions Bar to Mouse & Keys Scroll Panel
    $quickInputCard = New-Card -Title "Instant Input & Latency Actions" -X 4 -Y 660 -W 1020 -H 80 -AccentColor $Script:C.Sapphire

    $btnApplyAllInput = New-Btn "Apply All Input" 14 32 236 36 -BG $Script:C.Sapphire -FG $Script:C.Crust
    $btnApplyAllInput.Add_Click({
        $inputIds = @("INP01","INP02","INP03","INP04","INP05","INP06","INP07","INP08","INP09","INP10","INP11","INP12")
        Invoke-TweakList -Ids $inputIds
        foreach ($k in $inputTab.Cbs.Keys) { $inputTab.Cbs[$k].Checked = $true }
        Write-Status "All Mouse & Keyboard Latency Tweaks Applied!" $Script:C.Green
        [System.Windows.Forms.MessageBox]::Show("All 12 Mouse & Keyboard latency optimizations applied successfully!", "Input Latency", "OK", "Information") | Out-Null
    })

    $btnQuickFK = New-Btn "eSports FilterKeys" 268 32 236 36 -BG $Script:C.Teal -FG $Script:C.Crust
    $btnQuickFK.Add_Click({
        $fkKey = "HKCU:\Control Panel\Accessibility\Keyboard Response"
        Set-RegValue $fkKey "Flags" "27" -Type String -Desc "FilterKeys Flags 27"
        Set-RegValue $fkKey "DelayBeforeAcceptance" "0" -Type String -Desc "DelayBeforeAcceptance 0"
        Set-RegValue $fkKey "AutoRepeatDelay" "200" -Type String -Desc "AutoRepeatDelay 200"
        ...
        Write-Status "eSports FilterKeys Activated Instantly!" $Script:C.Green
        [System.Windows.Forms.MessageBox]::Show("eSports FilterKeys activated instantly (0ms delay, 200ms repeat delay, 20ms repeat rate) without reboot!", "FilterKeys", "OK", "Information") | Out-Null
    })

    $btnQuickUSB = New-Btn "Optimize USB Controllers" 522 32 236 36 -BG $Script:C.Gold -FG $Script:C.Crust
    $btnQuickUSB.Add_Click({
        Enable-UsbXhciMsiAndPriority -Priority "High"
        [System.Windows.Forms.MessageBox]::Show("USB xHCI Controller MSI Mode & High Priority configured successfully!", "USB Optimization", "OK", "Information") | Out-Null
    })

    $btnQuickResetInput = New-Btn "Reset Input Default" 776 32 236 36 -BG $Script:C.Red -FG $Script:C.Crust
    $btnQuickResetInput.Add_Click({
        $r = [System.Windows.Forms.MessageBox]::Show("Khôi phục toàn bộ cài đặt Chuột, Bàn phím & USB về mặc định của Windows?`n`nThao tác này sẽ hoàn tác tất cả tinh chỉnh input latency.", "Reset Input", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Warning)
        if ($r -eq [System.Windows.Forms.DialogResult]::Yes -or $r -eq "Yes") {
            Reset-InputToDefault
            [System.Windows.Forms.MessageBox]::Show("Đã khôi phục cài đặt Chuột, Bàn phím & USB về mặc định Windows thành công!", "Done", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information) | Out-Null
        }
    })

    $quickInputCard.Controls.AddRange(@($btnApplyAllInput, $btnQuickFK, $btnQuickUSB, $btnQuickResetInput))
    $inputTab.SP.Controls.Add($quickInputCard)

    $tc.TabPages.Add($inputTab.Tab)
    ...
    # TAB 9: SERVICES
    # =========================================================================
    $svcTab = Build-TweakTab "  Services  " "Services" $Script:C.Sapphire "Apply Services Tweaks"
    $tc.TabPages.Add($svcTab.Tab)
